...
class TestInteractiveIntegration:
    ...
    def test_interactive_with_pipeline(
        self, mock_response_gen, mock_aws_client, mock_query_parser
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            is_valid, warnings, suggestions = builder.validator.validate_query(
    ...
    def test_history_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            history_manager.add_to_history(
                "Invalid query", False, error_message="Parse error"
            )
    ...
    def test_favorites_with_validation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            builder.history_manager.add_favorite(
                "Good Query", good_query, "A well-formed query"
            )
            ...
            builder.history_manager.add_favorite(
                "Ambiguous Query", ambiguous_query, "Needs improvement"
            )
    ...
    def test_validation_with_common_patterns(self):
        ...
        test_queries = [
            ...
            (
                "How much did compute cost recently?",
                False,
                1,
            ),  # Ambiguous service and time
            (
                "Show me my AWS bill for July 2025",
                True,
                1,
            ),  # Good query but may have minor warnings
            (
                "Lambda costs this year broken down by service",
                True,
                0,
            ),  # Good detailed query
        ]
        ...
        for query, expected_valid, expected_min_warnings in test_queries:
            ...
            if expected_valid:
                ...
                assert (
                    len(warnings) <= expected_min_warnings
                ), f"Too many warnings for: {query}"
            else:
                assert (
                    len(warnings) >= expected_min_warnings
                ), f"Not enough warnings for: {query}"
    ...
    def test_template_coverage_for_requirements(self):
        ...
        assert any(
            "Monthly" in name or "Quarterly" in name or "Year" in name
