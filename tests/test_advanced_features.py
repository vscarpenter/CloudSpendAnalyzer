...
from src.aws_cost_cli.trend_analysis import (
    TrendAnalyzer,
    CostForecaster,
...
class TestTrendAnalyzer:
    ...
    def test_trend_analysis_stable(self):
        ...
        trend = self.analyzer.analyze_trend(
            stable_data, self.comparison_data, TrendAnalysisType.PERIOD_OVER_PERIOD
        )
                ...
                CostResult(
                    ...
                    groups=[
                        Group(
                            ...
                            metrics={
                                "BlendedCost": CostAmount(amount=Decimal("600.00"))
                            },
                        ),
                        Group(
                            keys=["Amazon Simple Storage Service"],
                            metrics={
                                "BlendedCost": CostAmount(amount=Decimal("400.00"))
                            },
                        ),
                    ],
                )
                ...
                CostResult(
                    ...
                    groups=[
                        Group(
                            keys=["Amazon Elastic Compute Cloud - Compute"],
                            metrics={
                                "BlendedCost": CostAmount(amount=Decimal("500.00"))
                            },
                        ),
                        Group(
                            keys=["Amazon Simple Storage Service"],
                            metrics={
                                "BlendedCost": CostAmount(amount=Decimal("300.00"))
                            },
                        ),
                    ],
                )
...
class TestCostForecaster:
    ...
    def setup_method(self):
        ...
        for i in range(6):  # 6 months of data
            ...
            month_end = (
                datetime(2025, i + 2, 1, tzinfo=timezone.utc)
                if i < 5
                else datetime(2025, 7, 1, tzinfo=timezone.utc)
            )
    ...
    def test_forecast_generation(self):
        ...
        forecasts = self.forecaster.forecast_costs(
            self.historical_data, forecast_months=3
        )
        ...
        for i, forecast in enumerate(forecasts):
            assert forecast.forecasted_amount.amount > Decimal(
                "1500"
            )  # Should be higher than last historical
            assert forecast.confidence_interval_lower.amount >= 0
            assert (
                forecast.confidence_interval_upper.amount
                > forecast.forecasted_amount.amount
            )
    ...
    def test_forecast_by_service(self):
                    ...
                    CostResult(
                        ...
                        groups=[
                            Group(
                                keys=["EC2"],
                                metrics={
                                    "BlendedCost": CostAmount(
                                        amount=Decimal(str(600 + i * 50))
                                    )
                                },
                            ),
                            Group(
        ...
        service_forecasts = self.forecaster.forecast_by_service(
            historical_with_services, forecast_months=2
        )
                ...
                CostResult(
                    ...
                    groups=[
                        Group(
                            keys=["EC2"],
                            metrics={
                                "BlendedCost": CostAmount(amount=Decimal("800.00"))
                            },
                        ),
                        Group(
                            keys=["S3"],
                            metrics={
                                "BlendedCost": CostAmount(amount=Decimal("200.00"))
                            },
                        ),
                    ],
                )
...
class TestAdvancedResponseFormatting:
    ...
    def test_simple_formatter_with_trend(self):
        ...
        response = formatter.format_response(
            self.cost_data_with_trend, "test query", query_params
        )
    ...
    def test_simple_formatter_with_forecast(self):
        ...
        response = formatter.format_response(
            self.cost_data_with_forecast, "test query", query_params
        )
    ...
    def test_rich_formatter_with_advanced_features(self):
        ...
        query_params = QueryParameters(
            trend_analysis=TrendAnalysisType.MONTH_OVER_MONTH, include_forecast=True
        )
