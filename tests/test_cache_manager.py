...
def sample_cost_data():
    ...
    group = Group(keys=["EC2-Instance"], metrics={"BlendedCost": cost_amount})

    cost_result = CostResult(
        time_period=time_period, total=cost_amount, groups=[group], estimated=False
    )
...
class TestCacheManager:
    ...
    def test_generate_query_hash_different_params(
        self, cache_manager, sample_query_params
    ):
    ...
    def test_generate_query_hash_different_profiles(
        self, cache_manager, sample_query_params
    ):
    ...
    def test_cache_and_retrieve_data(
    ...
    def test_cache_data_with_profile(
        self, cache_manager, sample_query_params, sample_cost_data
    ):
        ...
        # Cache with profile
        success = cache_manager.cache_data(
            sample_query_params, sample_cost_data, profile=profile
        )
        assert success is True

        # Retrieve with same profile
        retrieved_data = cache_manager.get_cached_data(
            sample_query_params, profile=profile
        )
        assert retrieved_data is not None

        # Retrieve with different profile should return None
        different_profile_data = cache_manager.get_cached_data(
            sample_query_params, profile="different"
        )
    ...
    def test_cache_expiration(
        ...
        short_ttl = 1
        success = cache_manager.cache_data(
            sample_query_params, sample_cost_data, ttl=short_ttl
        )
        assert success is True

        # Immediately retrieve - should work
        retrieved_data = cache_manager.get_cached_data(
            sample_query_params, ttl=short_ttl
        )
        assert retrieved_data is not None
    ...
    def test_cache_file_corruption_handling(
        self, cache_manager, sample_query_params, temp_cache_dir
    ):
    ...
    def test_cache_data_serialization_edge_cases(
        self, cache_manager, sample_query_params
    ):
    ...
    def test_cache_read_failure(
        self, cache_manager, sample_query_params, sample_cost_data, temp_cache_dir
    ):
