...
from src.aws_cost_cli.models import QueryParameters, TimePeriod, MetricType, TimePeriodGranularity
...
class TestCredentialManager:
    ...
    def test_get_available_profiles_exception(self, mock_session_class):
        ...
        type(mock_session).available_profiles = PropertyMock(side_effect=Exception("Error"))
        mock_session_class.return_value = mock_session
...
class TestAWSCostClient:
    ...
    def test_build_cost_request_with_service_filter(self, mock_session_class):
        ...
        assert request['Filter']['Dimensions']['Values'] == ['Amazon Elastic Compute Cloud - Compute']
    ...
    def test_get_cost_and_usage_retry_exhausted(self, mock_session_class):
        ...
        with patch('time.sleep'):  # Mock sleep to speed up test
            with pytest.raises(RuntimeError, match="AWS API rate limit exceeded after multiple retries"):
                client.get_cost_and_usage(params)
    ...
    def test_build_cost_request_default_time_period(self, mock_session_class):
        ...
        start_date = datetime.strptime(request['TimePeriod']['Start'], '%Y-%m-%d').date()
