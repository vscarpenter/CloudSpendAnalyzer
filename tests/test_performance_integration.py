...
from src.aws_cost_cli.models import (
    QueryParameters, CostData, CostResult, CostAmount, TimePeriod,
    TimePeriodGranularity, MetricType, Config
)
...
class TestPerformanceIntegration:
    ...
    def test_query_with_performance_optimizations(self, mock_query_parser, mock_aws_client_class):
        ...
        mock_aws_client.get_cost_and_usage_with_performance_optimization.return_value = mock_cost_data
        
        # Mock credential validation
        with patch('src.aws_cost_cli.query_pipeline.CredentialManager') as mock_cred_manager:
            ...
            with patch('src.aws_cost_cli.query_pipeline.ResponseGenerator') as mock_response_gen:
    ...
    @patch('src.aws_cost_cli.query_pipeline.AWSCostClient')
    @patch('src.aws_cost_cli.query_pipeline.QueryParser')
    def test_query_without_performance_optimizations(self, mock_query_parser, mock_aws_client_class):
    ...
    def test_large_query_chunking_logic(self):
        ...
        large_params = QueryParameters(
            time_period=TimePeriod(
                ...
                end=datetime(2025, 12, 31)  # 365 days
            ),
            granularity=TimePeriodGranularity.DAILY
        )
        ...
        small_params = QueryParameters(
            time_period=TimePeriod(
                start=datetime(2025, 7, 1),
                end=datetime(2025, 7, 31)  # 30 days
            ),
            granularity=TimePeriodGranularity.DAILY
        )
