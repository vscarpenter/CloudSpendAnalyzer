...
class TestQueryFavorite:
    ...
    def test_favorite_default_created_at(self):
        ...
        before = datetime.now()
        favorite = QueryFavorite(name="Test Favorite", query="Test query")
        after = datetime.now()
...
class TestQueryHistoryManager:
    ...
    def test_history_manager_initialization(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert manager.favorites_file == history_file.replace(
...
class TestQueryValidator:
    ...
    def test_validate_query_basic(self):
        ...
        # Test good query
        is_valid, warnings, suggestions = validator.validate_query(
    ...
    def test_validate_query_ambiguous_terms(self):
        ...
        is_valid, warnings, suggestions = validator.validate_query(
            "How much did storage cost last month?"
        )
        assert len(warnings) > 0
...
class TestInteractiveQueryBuilder:
    ...
    def test_get_query_suggestions(self):
        ...
        suggestions = builder.get_query_suggestions("EC2")
        assert any(
            "Custom EC2 query" in suggestions for _ in [True]
...
class TestIntegration:
    ...
    def test_full_workflow_with_templates(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Test template usage
            template = builder.template_manager.get_template_by_name(
                "EC2 Monthly Costs"
            )
            assert template is not None

            # Simulate using template
            query = template.template.replace("{month}", "July").replace(
                "{year}", "2025"
            )
    ...
    def test_validation_with_suggestions(self):
        ...
        assert any(
            ...
            for keyword in ["specific", "s3", "month", "year"]
        )
    ...
    def test_favorites_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            builder.history_manager.add_favorite(
                "Monthly EC2", query, "Monthly EC2 cost check"
            )
