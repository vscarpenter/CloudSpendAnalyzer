...
class TestPerformanceMonitor:
    ...
    def test_start_and_end_query(self):
        ...
        final_metrics = monitor.end_query(
            "test_query", api_calls_made=2, cache_hit=True
        )
    ...
    def test_compression_ratio_calculation(self):
        ...
        monitor.start_query("test_query")
        final_metrics = monitor.end_query(
            "test_query", compressed_cache_size=500, uncompressed_cache_size=1000
        )
...
class TestParallelQueryExecutor:
    ...
    def test_execute_parallel_queries(self, mock_executor_class):
        ...
        with patch("concurrent.futures.as_completed") as mock_as_completed:
            ...
            queries = [{"params": Mock()}, {"params": Mock()}]
...
class TestCompressedCacheManager:
    ...
    def test_cache_data_compressed(self, tmp_path):
        ...
        large_data = {
            "test": "data" * 1000,
            "results": [{"item": i} for i in range(100)],
        }
...
class TestPerformanceOptimizedClient:
    ...
    def test_get_cost_and_usage_optimized_cache_hit(self):
        ...
        mock_cost_data = Mock(spec=CostData)
        with patch.object(
            client.compressed_cache,
            "get_cached_data_compressed",
            return_value=mock_cost_data,
        ):
            params = QueryParameters()
    ...
    def test_get_cost_and_usage_optimized_no_cache(self):
        ...
        with patch.object(
            client.compressed_cache, "get_cached_data_compressed", return_value=None
        ), patch.object(
            client.compressed_cache,
            "cache_data_compressed",
            return_value={"compressed_size": 500, "uncompressed_size": 1000},
        ):
    ...
    def test_execute_parallel_query(self):
        ...
        with patch.object(
            client.parallel_executor, "split_large_query", return_value=mock_chunks
        ), patch.object(
            client.parallel_executor,
            "execute_parallel_queries",
            return_value=[mock_cost_data1, mock_cost_data2],
        ), patch.object(
            client.parallel_executor, "merge_cost_data", return_value=merged_data
        ):
    ...
    def test_get_performance_summary(self):
        ...
        with patch.object(
            client.monitor, "get_performance_summary", return_value={"test": "data"}
        ), patch.object(
            client.compressed_cache,
            "get_compression_stats",
            return_value={"compression": "stats"},
        ):
