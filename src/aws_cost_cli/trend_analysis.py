...
class TrendAnalyzer:
    ...
    def analyze_trend(
        ...
        if comparison_total.amount != 0:
            change_percentage = float(
                (change_amount.amount / comparison_total.amount) * 100
            )
        else:
    ...
    def analyze_service_trends(
        self, current_data: CostData, comparison_data: CostData
    ) -> Dict[str, TrendData]:
        """
        ...
        for service in all_services:
            current_cost = current_services.get(
                service, CostAmount(amount=Decimal("0"))
            )
            comparison_cost = comparison_services.get(
                service, CostAmount(amount=Decimal("0"))
            )
            ...
            for group in result.groups:
                if group.keys:  # Assuming first key is service name
                    ...
                    if service_name in service_costs:
                        ...
                        service_costs[service_name] = CostAmount(
                            amount=total_amount, unit=unit
                        )

        return service_costs

    def get_top_cost_changes(
        self, service_trends: Dict[str, TrendData], limit: int = 5
    ...
    def get_fastest_growing_services(
        ...
        growing_services = [
            (service, trend)
            for service, trend in service_trends.items()
            if trend.change_percentage > 0
        ]

        sorted_trends = sorted(
            growing_services, key=lambda x: x[1].change_percentage, reverse=True
        )
...
class CostForecaster:
    ...
    def forecast_costs(
        self, historical_data: List[CostData], forecast_months: int = 3
    ) -> List[ForecastData]:
        """
        ...
        if len(historical_data) < self.min_data_points:
            raise ValueError(
                f"Need at least {self.min_data_points} historical data points for forecasting"
            )
    ...
    def _extract_time_series(
        self, historical_data: List[CostData]
    ...
    def _forecast_single_period(
        self, time_series: List[Tuple[datetime, float]], periods_ahead: int
    ) -> ForecastData:
        """
        ...
        confidence_margin = max(
            2 * std_dev, forecasted_amount * 0.1
        )  # At least 10% margin
        ...
        return ForecastData(
            forecasted_amount=CostAmount(
                amount=Decimal(str(round(forecasted_amount, 2)))
            ),
            confidence_interval_lower=CostAmount(
                amount=Decimal(str(round(lower_bound, 2)))
            ),
            confidence_interval_upper=CostAmount(
                amount=Decimal(str(round(upper_bound, 2)))
            ),
    ...
    def _calculate_prediction_accuracy(
        self, time_series: List[Tuple[datetime, float]], slope: float, intercept: float
    ) -> float:
        """
        ...
        for service, time_series in service_time_series.items():
            if len(time_series) >= self.min_data_points:
                try:
                    forecasts = []
                    for month_offset in range(1, forecast_months + 1):
                        forecast = self._forecast_single_period(
                            time_series, month_offset
                        )
...
def calculate_cost_efficiency_metrics(cost_data: CostData) -> Dict[str, Any]:
    ...
    if cost_data.results and cost_data.results[0].groups:
        ...
        for result in cost_data.results:
            for group in result.groups:
                total_amount = sum(
                    float(cost_amount.amount) for cost_amount in group.metrics.values()
                )
        ...
        if service_costs:
            ...
            if total_cost > 0:
                ...
                metrics["top_service_percentage"] = (
                    max_service_cost / total_cost
                ) * 100
