...
class CostOptimizer:
    ...
    def generate_optimization_report(self, analysis_period: Optional[TimePeriod] = None) -> OptimizationReport:
        ...
        rightsizing_recommendations = self._analyze_rightsizing_opportunities(analysis_period)
        ...
        total_savings = sum(
            (rec.potential_savings.amount for rec in recommendations),
            Decimal('0.0')
        )
    ...
    def _analyze_unused_resources(self, period: TimePeriod) -> List[OptimizationRecommendation]:
        ...
        try:
            ...
            for volume in unused_volumes:
                recommendations.append(OptimizationRecommendation(
                    ...
                    potential_savings=CostAmount(amount=Decimal(str(volume['estimated_monthly_cost']))),
                    ...
                    metadata={"volume_size": volume.get('Size'), "volume_type": volume.get('VolumeType')}
                ))
            ...
            for eip in unused_eips:
                recommendations.append(OptimizationRecommendation(
                    ...
                    potential_savings=CostAmount(amount=Decimal('3.65')),  # ~$0.005/hour * 24 * 30
            ...
            for instance in idle_rds:
                recommendations.append(OptimizationRecommendation(
                    ...
                    potential_savings=CostAmount(amount=Decimal(str(instance['estimated_monthly_cost']))),
    ...
    def _analyze_rightsizing_opportunities(self, period: TimePeriod) -> List[OptimizationRecommendation]:
                ...
                if modify_rec:
                    ...
                    if target_instances:
                        ...
                        if modify_rec.get('EstimatedMonthlySavings'):
                            estimated_savings = Decimal(modify_rec['EstimatedMonthlySavings'])
                        
                        recommendations.append(OptimizationRecommendation(
                            type=OptimizationType.RIGHTSIZING,
                            severity=SeverityLevel.MEDIUM if estimated_savings > 50 else SeverityLevel.LOW,
                            ...
                            metadata={
                                ...
                                "utilization": current_instance.get('UtilizationMetrics', {})
                            }
                        ))
    ...
    def _get_reserved_instance_recommendations(self) -> List[OptimizationRecommendation]:
        ...
        try:
            ...
            for rec in response.get('Recommendations', []):
                ...
                if details.get('EstimatedMonthlySavingsAmount'):
                    estimated_savings = Decimal(details['EstimatedMonthlySavingsAmount'])
                
                instance_details = details.get('InstanceDetails', {}).get('EC2InstanceDetails', {})
                
                recommendations.append(OptimizationRecommendation(
                    type=OptimizationType.RESERVED_INSTANCES,
                    severity=SeverityLevel.MEDIUM if estimated_savings > 100 else SeverityLevel.LOW,
                    ...
                    metadata={
                        ...
                        "recommended_quantity": details.get('RecommendedNumberOfInstancesToPurchase')
                    }
                ))
        ...
        try:
            ...
            for rec in response.get('SavingsPlansRecommendations', []):
                ...
                recommendations.append(OptimizationRecommendation(
                    ...
                    severity=SeverityLevel.MEDIUM if estimated_savings > 200 else SeverityLevel.LOW,
            ...
            for anomaly in response.get('Anomalies', []):
                ...
                if max_impact > 10:  # Only report significant anomalies
                    anomalies.append(CostAnomaly(
                        ...
                        anomaly_date=datetime.strptime(anomaly.get('AnomalyStartDate', ''), '%Y-%m-%d'),
                        expected_cost=CostAmount(amount=Decimal('0')),  # AWS doesn't provide expected cost
                        actual_cost=CostAmount(amount=max_impact),
                        variance_percentage=float(impact.get('TotalImpact', 0)),
                        severity=SeverityLevel.HIGH if max_impact > 100 else SeverityLevel.MEDIUM,
                        description=f"Cost anomaly detected: {anomaly.get('AnomalyScore', {}).get('CurrentScore', 0):.1f} anomaly score",
                        root_cause_analysis=self._analyze_anomaly_root_cause(anomaly)
                    ))
    ...
    def _analyze_budget_variances(self, period: TimePeriod) -> List[BudgetVariance]:
        ...
        try:
            ...
            for budget in response.get('Budgets', []):
                ...
                variance_percentage = float((variance_amount / budgeted_amount) * 100) if budgeted_amount > 0 else 0
    ...
    def _get_actual_spending_for_budget(self, budget: Dict[str, Any], period: TimePeriod) -> Decimal:
