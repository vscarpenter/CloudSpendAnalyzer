    ...
    def end_query(self, query_id: str, **kwargs) -> Optional[QueryMetrics]:
        ...
        with self._lock:
            ...
            if metrics.compressed_cache_size and metrics.uncompressed_cache_size:
                metrics.compression_ratio = metrics.compressed_cache_size / metrics.uncompressed_cache_size
...
class ParallelQueryExecutor:
    ...
    def execute_parallel_queries(self, queries: List[Dict[str, Any]], 
                                aws_client, use_cache: bool = True) -> List[CostData]:
        """
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_index, timeout=self.timeout):
                try:
                    ...
                    results[index] = AWSAPIError(f"Query {index} timed out after {self.timeout} seconds")
                except Exception as e:
                    index = future_to_index[future]
                    results[index] = e
    ...
    def split_large_query(self, params: QueryParameters, max_days_per_chunk: int = 90) -> List[QueryParameters]:
        """
    ...
    def merge_cost_data(self, cost_data_list: List[CostData]) -> CostData:
        ...
        return CostData(
            ...
            total_cost=template.total_cost.__class__(amount=total_amount, unit=template.total_cost.unit),
...
class QueryPaginator:
    ...
    def paginate_time_series(self, params: QueryParameters, 
                           aws_client, page_size: Optional[int] = None) -> List[CostData]:
        """
    ...
    def get_page_summary(self, pages: List[CostData]) -> Dict[str, Any]:
        ...
        total_cost = sum(float(page.total_cost.amount) for page in pages if not hasattr(page, 'error'))
        ...
        return {
            ...
            "success_rate": (total_pages - error_pages) / total_pages if total_pages > 0 else 0
        }
...
class CompressedCacheManager:
    ...
    def cache_data_compressed(self, cache_key: str, data: CostData, 
        ...
        serialized_data = json.dumps(self.base_cache._serialize_cost_data(data), default=str)
    ...
    def get_compression_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats['total_space_saved'] = stats['total_uncompressed_size'] - stats['total_compressed_size']
            
            if compression_ratios:
                stats['average_compression_ratio'] = sum(compression_ratios) / len(compression_ratios)
                stats['space_saved_percent'] = (1 - stats['average_compression_ratio']) * 100
...
class PerformanceOptimizedClient:
    ...
    def __init__(self, aws_client, cache_manager=None, 
                 enable_parallel: bool = True, enable_compression: bool = True,
    ...
    def get_cost_and_usage_optimized(self, params: QueryParameters, 
                                   use_cache: bool = True,
        ...
        try:
            ...
            if use_cache and self.compressed_cache:
                ...
                cached_data = self.compressed_cache.get_cached_data_compressed(cache_key)
                if cached_data:
                    if self.enable_monitoring:
                        self.monitor.end_query(query_id, cache_hit=True, api_calls_made=0)
                    return cached_data
            ...
            if use_cache and self.compressed_cache:
                ...
                compression_stats = self.compressed_cache.cache_data_compressed(cache_key, result)
                
                if self.enable_monitoring:
                    self.monitor.update_query_metrics(
                        query_id,
                        compressed_cache_size=compression_stats.get('compressed_size'),
                        uncompressed_cache_size=compression_stats.get('uncompressed_size')
                    )
    ...
    def _execute_parallel_query(self, params: QueryParameters, 
                              use_cache: bool, query_id: str) -> CostData:
        ...
        if len(chunks) == 1:
            ...
            if self.enable_monitoring:
                self.monitor.update_query_metrics(query_id, api_calls_made=1, parallel_requests=1)
            return result
