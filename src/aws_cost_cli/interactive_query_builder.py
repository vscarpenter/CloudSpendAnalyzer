...
class QueryHistoryManager:
    ...
    def __init__(self, history_file: str = None):
        ...
        self.favorites_file = self.history_file.replace(
    ...
    def add_favorite(self, name: str, query: str, description: str = None):
        ...
        favorite = QueryFavorite(name=name, query=query, description=description)
...
class QueryValidator:
    ...
    def __init__(self, query_parser: QueryParser = None):
        ...
        self.common_issues = {
            "ambiguous_service": ["storage", "compute", "database", "networking"],
            "ambiguous_time": ["recently", "a while ago", "some time ago"],
            "missing_context": ["costs", "spending", "bill"],
        }
    ...
    def validate_query(self, query: str) -> Tuple[bool, List[str], List[str]]:
        ...
        for ambiguous in self.common_issues["ambiguous_service"]:
            if ambiguous in query_lower:
                warnings.append(
                    f"'{ambiguous}' is ambiguous - consider specifying exact service (EC2, S3, RDS, etc.)"
                )
                suggestions.append(
                    f"Try: '{query.replace(ambiguous, 'EC2')}' or similar with specific service"
                )
        ...
        for ambiguous in self.common_issues["ambiguous_time"]:
            if ambiguous in query_lower:
                warnings.append(
                    f"'{ambiguous}' is vague - consider specific time periods"
                )
        ...
        has_service = any(
            ...
            for service in ["ec2", "s3", "rds", "lambda", "cloudfront"]
        )
        has_time = any(
            time_ref in query_lower
            for time_ref in [
                "month",
                "year",
                "day",
        ...
        has_cost_keyword = any(
            keyword in query_lower
            for keyword in ["cost", "spend", "bill", "charge", "price"]
        )

        if not has_cost_keyword:
            warnings.append(
                "Query doesn't mention costs - consider adding 'cost', 'spending', or 'bill'"
            )
            suggestions.append(
                f"Try: '{query} costs' or 'How much did I spend on {query}?'"
            )

        if not has_time and not has_service:
            warnings.append("Query lacks both service and time context")
            suggestions.append(
                "Try: 'EC2 costs last month' or 'Total spending this year'"
            )
            ...
            and not has_service
            and not any(
                word in query_lower for word in ["total", "all", "overall", "entire"]
            )
        ):
            warnings.append(
                "Query lacks service context - consider specifying which service or use 'total'"
            )
            suggestions.append(
                "Try: 'EC2 costs last month' or 'Total AWS costs last month'"
            )
        ...
        if self.query_parser:
            try:
                ...
                if not parsed.get("service") and not parsed.get("start_date"):
                    ...
                    suggestions.append(
                        "Consider being more specific about service or time period"
                    )
            except Exception:
                is_valid = False
                warnings.append(
                    "Query could not be parsed - may be too complex or unclear"
                )
...
class InteractiveQueryBuilder:
    ...
    def _use_template(self) -> Optional[str]:
        ...
        self.console.print(
            Panel(Text("📋 Query Templates", style="bold green"), border_style="green")
        )
    ...
    def _browse_history(self) -> Optional[str]:
        ...
        self.console.print(
            Panel(Text("📚 Query History", style="bold green"), border_style="green")
        )
    ...
    def _validate_query(self):
        ...
        self.console.print(
            Panel(Text("🔍 Query Validation", style="bold green"), border_style="green")
        )
    ...
    def get_query_suggestions(self, partial_query: str = "") -> List[str]:
        ...
        if partial_query:
            ...
            for template in self.template_manager.templates:
                if (
                    partial_lower in template.name.lower()
                    or partial_lower in template.description.lower()
                    or partial_lower in template.template.lower()
                ):
