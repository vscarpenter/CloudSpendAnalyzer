...
class AWSCredentialsError(AWSCostCLIError):
    ...
    def __init__(self, message: str = "AWS credentials not found or invalid", profile: Optional[str] = None):
        suggestions = [
        ...
        if profile:
            ...
            suggestions.append(f"Run 'aws configure --profile {profile}' to set up the profile")
...
class AWSPermissionsError(AWSCostCLIError):
    ...
    def __init__(self, message: str = "Insufficient AWS permissions", required_permissions: Optional[List[str]] = None):
...
class AWSAPIError(AWSCostCLIError):
    ...
    def __init__(self, message: str, aws_error_code: Optional[str] = None, aws_error_message: Optional[str] = None):
...
class QueryParsingError(AWSCostCLIError):
    ...
    def __init__(self, message: str = "Failed to parse natural language query", original_query: Optional[str] = None):
...
class LLMProviderError(AWSCostCLIError):
    ...
    def __init__(self, message: str, provider: Optional[str] = None, fallback_available: bool = True):
...
class ConfigurationError(AWSCostCLIError):
    ...
    def __init__(self, message: str = "Configuration error", config_file: Optional[str] = None):
...
def format_error_message(error: AWSCostCLIError, include_suggestions: bool = True) -> str:
    """
...
def handle_aws_client_error(client_error, operation: str = "AWS operation") -> AWSCostCLIError:
    """
...
def handle_network_error(network_error, operation: str = "network operation") -> NetworkError:
    """
...
def is_retryable_error(error: AWSCostCLIError) -> bool:
    ...
    if isinstance(error, AWSAPIError):
        return error.aws_error_code in ['ThrottlingException', 'ServiceUnavailable', 'Throttling']
