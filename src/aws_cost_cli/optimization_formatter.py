...
class OptimizationFormatter:
    ...
    def _format_report_header(self, report: OptimizationReport) -> str:
        ...
        with self.console.capture() as capture:
            ...
            self.console.print(
                f"📊 Analysis Period: {report.analysis_period.start.strftime('%Y-%m-%d')} to {report.analysis_period.end.strftime('%Y-%m-%d')}"
            )
            self.console.print(
                f"💰 Total Potential Savings: ${report.total_potential_savings.amount:,.2f}/month"
            )
    ...
    def _format_executive_summary(self, report: OptimizationReport) -> str:
        ...
        with self.console.capture() as capture:
            self.console.print(
                Panel(
                    Text("Executive Summary", style="bold green"), border_style="green"
                )
            )
            ...
            budget_issues = len(
                [b for b in report.budget_variances if b.is_over_budget]
            )
            ...
            summary_table.add_row(
                ...
                f"${report.total_potential_savings.amount:,.2f}",
            )
    ...
    def _format_recommendations_by_type(
        ...
        with self.console.capture() as capture:
            ...
            for rec_type in by_type:
                by_type[rec_type].sort(
                    key=lambda x: x.potential_savings.amount, reverse=True
                )
            ...
            for rec_type, recs in by_type.items():
                ...
                self.console.print(
                    f"   💰 Potential Savings: ${type_savings:,.2f}/month"
                )
                ...
                for i, rec in enumerate(recs[:3]):  # Show top 3
                    ...
                    self.console.print(
                        f"      💵 ${rec.potential_savings.amount:,.2f}/month"
                    )
                ...
                if len(recs) > 3:
                    self.console.print(
                        f"   ... and {len(recs) - 3} more recommendations"
                    )
    ...
    def _format_anomalies(self, anomalies: List[CostAnomaly]) -> str:
        ...
        with self.console.capture() as capture:
            ...
            significant_anomalies = [
                a for a in sorted_anomalies if a.actual_cost.amount > 100
            ]
            if significant_anomalies:
                self.console.print("\n🔍 Root Cause Analysis:")
                for anomaly in significant_anomalies[:3]:  # Show top 3
                    self.console.print(
                        f"   • {anomaly.service}: {anomaly.root_cause_analysis or 'Investigation needed'}"
                    )
    ...
    def _format_budget_variances(self, variances: List[BudgetVariance]) -> str:
        ...
        with self.console.capture() as capture:
            ...
            sorted_variances = sorted(
                variances, key=lambda x: abs(x.variance_percentage), reverse=True
            )
            ...
            if over_budget:
                ...
                for variance in over_budget[:3]:  # Show top 3
                    self.console.print(
                        f"   • {variance.budget_name}: Review spending patterns and adjust budget or implement cost controls"
                    )
    ...
    def _format_action_items(self, report: OptimizationReport) -> str:
        ...
        with self.console.capture() as capture:
            ...
            # Critical anomalies
            critical_anomalies = [
                a for a in report.anomalies if a.severity == SeverityLevel.CRITICAL
            ]
            ...
            budget_overruns = [
                b
                for b in report.budget_variances
                if b.is_over_budget and b.variance_percentage > 20
            ]
            ...
            if action_items:
                ...
                for i, item in enumerate(action_items[:10], 1):  # Show top 10
                    priority_text = (
                        "🔴 CRITICAL" if item["priority"] == 1 else "🟡 HIGH"
                    )
    ...
    def _generate_key_insights(self, report: OptimizationReport) -> List[str]:
        ...
        if report.total_potential_savings.amount > 1000:
            insights.append(
                f"Significant savings opportunity: ${report.total_potential_savings.amount:,.2f}/month potential savings identified"
            )
        ...
        unused_recs = [
            r
            for r in report.recommendations
            if r.type == OptimizationType.UNUSED_RESOURCES
        ]
        if unused_recs:
            unused_savings = sum(r.potential_savings.amount for r in unused_recs)
            insights.append(
                f"${unused_savings:,.2f}/month can be saved by removing {len(unused_recs)} unused resources"
            )

        # Rightsizing insight
        rightsizing_recs = [
            r for r in report.recommendations if r.type == OptimizationType.RIGHTSIZING
        ]
        if rightsizing_recs:
            rightsizing_savings = sum(
                r.potential_savings.amount for r in rightsizing_recs
            )
            insights.append(
                f"${rightsizing_savings:,.2f}/month potential savings from rightsizing {len(rightsizing_recs)} resources"
            )
        ...
        if report.anomalies:
            ...
            insights.append(
                f"${total_anomaly_impact:,.2f} in unexpected costs detected across {len(report.anomalies)} anomalies"
            )
        ...
        if over_budget:
            insights.append(
                f"{len(over_budget)} budget(s) exceeded - review spending controls"
            )
    ...
    def format_recommendations_summary(
        ...
        with self.console.capture() as capture:
            ...
            self.console.print(
                f"📋 {len(recommendations)} recommendations ({high_priority} high priority)"
            )

            # Show top 3 recommendations
            sorted_recs = sorted(
                recommendations, key=lambda x: x.potential_savings.amount, reverse=True
            )
            self.console.print("\n🔝 Top Recommendations:")
