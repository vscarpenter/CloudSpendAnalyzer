...
class CacheManager:
    ...
    def _generate_query_hash(
        self, params: QueryParameters, profile: Optional[str] = None
    ) -> str:
        """
        ...
        cache_key = {"params": params_dict, "profile": profile or "default"}
    ...
    def _deserialize_cost_data(self, data_dict: Dict[str, Any]) -> CostData:
        ...
        def reconstruct_cost_amount(ca_dict):
            if ca_dict is None:
                return None
            return CostAmount(
                amount=Decimal(str(ca_dict["amount"])), unit=ca_dict["unit"]
            )
        ...
        def reconstruct_group(group_dict):
            ...
            return Group(keys=group_dict["keys"], metrics=metrics)
