...
def cli_error_boundary(message: str, title: str = "Error"):
    """Decorator that renders unhandled exceptions as a consistent error panel.

    Replaces the catch-all ``except Exception`` tail that was repeated across
    most commands. On an unhandled exception it prints a red panel whose body is
    ``❌ {message}: {error}`` and whose title is ``title``, then exits with code
    1 -- exactly matching the inline blocks it replaces.

    ``SystemExit`` (e.g. ``sys.exit`` from inside the command, including bespoke
    per-error-type branches such as the AWS credential/permission handlers) is
    re-raised unchanged so those exit codes and messages are preserved. When
    debug mode is enabled (``ctx.obj['debug']``) the traceback is also printed,
    matching the commands that previously did so in their generic tail.
    """
    ...
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                console.print(
                    Panel(
                        Text(f"❌ {message}: {str(e)}", style="bold red"),
                        title=title,
                        border_style="red",
                    )
                )
...
@click.pass_context
@cli_error_boundary("Unexpected error", "Error")
def query(
    ctx,
    query: str,
    ...
    try:
        ...
        if result.success:
            if context.output_format == "json":
                # JSON output for programmatic use. CostData owns the
                # serialization of its own structure; the CLI only wraps it in
                # the request envelope (query/success/metadata).
                cost_dict = result.cost_data.to_dict()
                output = {
                    "query": query,
                    "success": True,
                    "total_cost": cost_dict["total_cost"],
                    "time_period": cost_dict["time_period"],
                    "metadata": result.metadata,
                    "results": cost_dict["results"],
                }
...
@cli_error_boundary("Failed to get suggestions", "Error")
...
@cli_error_boundary("Interactive session failed", "Error")
...
@cli_error_boundary("Favorites operation failed", "Error")
def favorites(
    action: str,
    name: Optional[str],
    query: Optional[str],
...
@cli_error_boundary("Failed to get pipeline status", "Error")
...
@cli_error_boundary("Configuration failed", "Configuration Error")
def configure(
    provider: str,
...
@cli_error_boundary("Failed to list profiles", "Error")
...
@cli_error_boundary("Failed to show configuration", "Error")
...
@cli_error_boundary("Failed to clear cache", "Error")
...
@cli_error_boundary("Failed to get cache statistics", "Error")
...
@cli_error_boundary("Failed to warm cache", "Error")
...
@cli.command()
@cli_error_boundary("Failed to cleanup cache", "Error")
def cleanup_cache():
...
@click.pass_context
@cli_error_boundary("Export failed", "Export Error")
def export(
    ctx,
    query: str,
    export_format: str,
...
@click.pass_context
@cli_error_boundary("Optimization analysis failed", "Error")
def optimize(
    ctx,
    profile: Optional[str],
    days: int,
    ...
    try:
        ...
        if output_format.lower() == "json":
            # JSON output for programmatic use. The report owns its own
            # serialization (including the derived summary counts).
...
@click.pass_context
@cli_error_boundary("Anomaly detection failed", "Error")
...
@cli_error_boundary("Failed to list providers", "Error")
...
@cli_error_boundary("Provider test failed", "Test Error")
...
@cli_error_boundary("Failed to get performance metrics", "Performance Error")
...
@cli_error_boundary("Health check failed", "Health Check Error")
...
@cli_error_boundary("Failed to reset metrics", "Reset Error")
