...
@click.pass_context
def query(ctx, query: str, profile: Optional[str], fresh: bool, output_format: Optional[str], 
          config_file: Optional[str], parallel: bool, compression: bool, 
          max_chunk_days: int, performance_metrics: bool):
            ...
            if context.output_format == 'json':
                ...
                if context.show_performance_metrics:
                    ...
                    if result.compression_stats:
                        ...
                        console.print(f"   Compression ratio: {stats.get('average_compression_ratio', 0):.2f}")
                        console.print(f"   Space saved: {stats.get('space_saved_percent', 0):.1f}%")
                    ...
                    if result.performance_metrics:
                        ...
                        if 'query_performance' in perf:
                            ...
                            if 'cache_hit_rate' in qp:
                                console.print(f"   Cache hit rate: {qp['cache_hit_rate']:.1%}")
                            if 'performance' in qp and 'avg_duration_ms' in qp['performance']:
                                console.print(f"   Avg query time: {qp['performance']['avg_duration_ms']:.1f}ms")
        else:
            # Handle error
            error = result.error
        ...
        if query:
            ...
            if result.success:
                ...
                if Confirm.ask("\n⭐ Save this query as a favorite?", default=False):
                    ...
                    try:
                        builder.history_manager.add_favorite(name, query, description or None)
...
@click.option(
    '--name',
    help='Name of the favorite (for add/remove/run actions)'
)
@click.option(
    '--query',
    help='Query to save as favorite (for add action)'
)
@click.option(
    '--description',
    help='Description for the favorite (for add action)'
)
@click.option(
    '--config-file', '-c',
    type=click.Path(exists=True),
...
def favorites(action: str, name: Optional[str], query: Optional[str], description: Optional[str], config_file: Optional[str]):
    ...
    try:
        ...
        if action == 'list':
            ...
            console.print(Panel(
                Text("⭐ Favorite Queries", style="bold blue"),
                border_style="blue"
            ))
...
@click.option(
    '--provider',
    type=click.Choice(['openai', 'anthropic', 'bedrock', 'ollama'], case_sensitive=False),
    required=True,
    help='LLM provider to configure'
)
@click.option(
    '--api-key',
    help='API key for the LLM provider (not needed for Ollama or Bedrock)'
)
@click.option(
    '--model',
...
def configure(provider: str, api_key: Optional[str], model: Optional[str], base_url: Optional[str], region: Optional[str], profile: Optional[str], config_file: Optional[str]):
    ...
    try:
        ...
        if output_format == 'json':
            ...
            if 'total_queries' in perf_summary:
                ...
                console.print(f"   Cache hit rate: {perf_summary.get('cache_hit_rate', 0):.1%}")
                console.print(f"   Error rate: {perf_summary.get('error_rate', 0):.1%}")
                console.print(f"   Total API calls: {perf_summary.get('total_api_calls', 0)}")
                
                if 'performance' in perf_summary:
                    perf = perf_summary['performance']
                    console.print(f"   Avg response time: {perf.get('avg_duration_ms', 0):.1f}ms")
                    console.print(f"   95th percentile: {perf.get('p95_duration_ms', 0):.1f}ms")
            else:
            ...
            console.print(f"   Expired entries: {cache_stats.get('expired_entries', 0)}")
            console.print(f"   Cache size: {cache_stats.get('cache_size_bytes', 0) / 1024 / 1024:.1f} MB")
            
            console.print(f"\n🗜️  Compression Statistics")
            console.print(f"   Compressed files: {compression_stats.get('compressed_files', 0)}")
            console.print(f"   Avg compression ratio: {compression_stats.get('average_compression_ratio', 0):.2f}")
            console.print(f"   Space saved: {compression_stats.get('space_saved_percent', 0):.1f}%")
            console.print(f"   Total space saved: {compression_stats.get('total_space_saved', 0) / 1024 / 1024:.1f} MB")
...
def export(ctx, query: str, export_format: str, output: Optional[str], 
          profile: Optional[str], fresh: bool, config_file: Optional[str]):
    ...
    try:
        ...
        if not output:
            ...
            service_part = f"_{result.query_params.service.lower()}" if result.query_params.service else ""
            output = f"aws_cost_export{service_part}_{timestamp}.{export_format.lower()}"
        ...
        if export_format.lower() not in export_manager.get_available_formats():
            ...
            console.print(Panel(
                Text(f"Export format '{export_format}' is not available. Available formats: {available}", style="bold red"),
                title="Export Error",
                border_style="red"
            ))
            
            if export_format.lower() == 'excel':
                console.print("💡 To enable Excel export, install openpyxl: pip install openpyxl")
        ...
        console.print(f"📅 Period: {result.cost_data.time_period.start.date()} to {result.cost_data.time_period.end.date()}")
...
@click.pass_context
def email_report(ctx, query: str, recipients: str, export_format: str, 
                profile: Optional[str], fresh: bool, config_file: Optional[str]):
    ...
    try:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        service_part = f"_{result.query_params.service.lower()}" if result.query_params.service else ""
        report_file = f"aws_cost_report{service_part}_{timestamp}.{export_format.lower()}"
        ...
        console.print(Panel(
            Text("✅ Email report functionality requires email configuration", style="bold yellow"),
            title="Email Report",
            border_style="yellow"
        ))
...
@click.pass_context
def optimize(ctx, profile: Optional[str], days: int, output_format: str, config_file: Optional[str]):
        ...
        if output_format.lower() == 'json':
            ...
            output = {
                ...
                'summary': {
                    ...
                    'high_priority_recommendations': len([r for r in report.recommendations if r.severity.value in ['high', 'critical']]),
            ...
            if report.total_potential_savings.amount > 0:
                console.print(Panel(
                    Text(f"💰 Total Monthly Savings Potential: ${report.total_potential_savings.amount:,.2f}", 
                ...
                console.print(Panel(
                    Text("✅ No significant optimization opportunities found", 
                         style="bold green", justify="center"),
                    border_style="green",
                    padding=(1, 2)
                ))
...
@click.option(
    '--service', '-s',
    help='Filter by specific AWS service'
)
@click.option(
    '--days', '-d',
    type=int,
...
@click.pass_context
def detect_anomalies(ctx, profile: Optional[str], service: Optional[str], days: int, threshold: float, config_file: Optional[str]):
    ...
    try:
        ...
        if not anomalies:
            console.print(Panel(
                Text("✅ No significant cost anomalies detected", style="bold green"),
        ...
        # Sort by cost impact
        sorted_anomalies = sorted(anomalies, key=lambda x: x.actual_cost.amount, reverse=True)
        
        for anomaly in sorted_anomalies:
            severity_style = "red" if anomaly.severity.value in ['high', 'critical'] else "yellow"
        ...
        if high_impact_anomalies:
            ...
            console.print("   • Consider setting up AWS Budgets alerts for early detection")
            console.print("   • Enable AWS Cost Anomaly Detection for automated monitoring")
...
@click.option(
    '--subject', '-s',
    help='Email subject (auto-generated if not specified)'
)
@click.option(
    '--attachments',
    type=click.Choice(['csv', 'json', 'excel'], case_sensitive=False),
...
@click.pass_context
def email_report(ctx, query: str, recipients: str, subject: Optional[str], 
                attachments: tuple, smtp_host: str, smtp_port: int, 
                smtp_username: str, smtp_password: str, no_tls: bool,
                profile: Optional[str], fresh: bool, config_file: Optional[str]):
    ...
    try:
        ...
        # Send email report
        console.print(f"📧 Sending email report to {len(recipient_list)} recipient(s)...")
        
        export_manager = ExportManager()
        
        # Filter available attachment formats
        available_formats = export_manager.get_available_formats()
        attachment_formats = [fmt for fmt in attachments if fmt.lower() in available_formats]
        
        if len(attachment_formats) != len(attachments):
            missing = [fmt for fmt in attachments if fmt.lower() not in available_formats]
            console.print(f"⚠️  Skipping unavailable formats: {', '.join(missing)}")
            if 'excel' in missing:
                console.print("💡 To enable Excel attachments, install openpyxl: pip install openpyxl")
        ...
        if success:
            ...
            if attachment_formats:
                console.print(f"📎 Attachments: {', '.join(attachment_formats).upper()}")
        else:
            console.print(Panel(
