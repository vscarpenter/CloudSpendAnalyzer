...
class QueryValidator:
    ...
    VALID_SERVICE_NAMES = {
        ...
        'elastic search', 'amazon elasticsearch service',
    ...
    def validate_query_parameters(self, params: QueryParameters) -> ValidationResult:
        ...
        # Validate date range
        date_errors, date_warnings = self._validate_date_range(params.time_period, params.granularity)
    ...
    def validate_raw_query(self, query: str) -> ValidationResult:
        ...
        for pattern in self.sql_patterns:
            if pattern.search(query):
                errors.append(f"Potentially malicious pattern detected in query: {pattern.pattern}")
        
        # Check for suspicious characters
        if any(char in query for char in ['<', '>', '{', '}', '`']):
            warnings.append("Query contains unusual characters that might cause parsing issues")
    ...
    def _validate_date_range(self, time_period: Optional[TimePeriod], granularity: Optional[str]) -> tuple[List[str], List[str]]:
        ...
        if date_range_days > max_days:
            errors.append(f"Date range too large for {granularity} granularity. Maximum: {max_days} days, requested: {date_range_days} days")
        ...
        if start_date < oldest_supported:
            warnings.append(f"Start date is very old ({start_date}). AWS Cost Explorer may have limited data availability")
        
        # Warn about large date ranges that might be slow
        if date_range_days > 90:
            warnings.append(f"Large date range ({date_range_days} days) may result in slow queries")
    ...
    def _validate_services(self, services: List[str]) -> List[str]:
        ...
        if len(services) > 20:
            errors.append(f"Too many services specified ({len(services)}). Consider using fewer services for better performance")
    ...
    def _validate_metrics(self, metrics: List[str]) -> List[str]:
        ...
        for metric in metrics:
            if metric not in valid_metrics:
                errors.append(f"Invalid metric type: {metric}. Valid options: {', '.join(valid_metrics)}")
    ...
    def _validate_group_by(self, group_by: List[str]) -> List[str]:
        ...
        valid_dimensions = {
            'SERVICE', 'AZ', 'INSTANCE_TYPE', 'REGION', 'USAGE_TYPE',
            'OPERATION', 'PURCHASE_TYPE', 'RECORD_TYPE', 'RESOURCE_ID',
            'LINKED_ACCOUNT', 'TENANCY', 'PLATFORM', 'DATABASE_ENGINE',
        }
        ...
        if len(group_by) > 5:
            errors.append(f"Too many group by dimensions ({len(group_by)}). AWS Cost Explorer supports up to 2 group by dimensions")
            ...
            if isinstance(field_value, str):
                for pattern in self.sql_patterns:
                    if pattern.search(field_value):
                        errors.append(f"Potentially malicious content detected: {field_value[:50]}")
                        break
    ...
    def _check_performance_implications(self, params: QueryParameters) -> List[str]:
        ...
        if params.time_period:
            ...
            if date_range_days > 90 and params.granularity == 'DAILY':
                warnings.append("Daily granularity with large date range may be slow. Consider using MONTHLY granularity")
        
        # Many services with grouping
        if params.services and len(params.services) > 10 and params.group_by:
            warnings.append("Querying many services with grouping may result in large result sets")
        
        # Multiple group by dimensions
        if params.group_by and len(params.group_by) > 1:
            warnings.append("Multiple group by dimensions may result in large result sets and slower queries")
...
class ValidationMiddleware:
    ...
    def validate_and_process(self, query: str, params: Optional[QueryParameters] = None) -> ValidationResult:
