...
class QueryPipeline:
    ...
    def process_query(self, context: QueryContext) -> QueryResult:
        ...
        try:
            ...
            formatted_response = self._generate_response(cost_data, query_params, context, result)
    ...
    def _parse_query(self, context: QueryContext, result: QueryResult) -> QueryParameters:
        ...
        try:
            ...
            try:
                ...
                query_params = self.query_parser._convert_to_query_parameters(parsed_result)
            ...
            raise QueryParsingError(f"Query parsing failed: {str(e)}", original_query=context.original_query)
    
    def _validate_parameters(self, query_params: QueryParameters, context: QueryContext, result: QueryResult):
    ...
    def _fetch_cost_data(self, query_params: QueryParameters, context: QueryContext, result: QueryResult) -> CostData:
        ...
        if use_cache:
            try:
                cache_key = self.cache_manager.generate_cache_key(query_params, context.profile or 'default')
    ...
    def _generate_response(self, cost_data: CostData, query_params: QueryParameters, 
                          context: QueryContext, result: QueryResult) -> str:
        ...
        if not self.response_generator:
            ...
            if context.output_format == "llm" and self.config.llm_config.get('provider'):
                try:
                    ...
                    if provider_type == 'openai':
                        ...
                        llm_provider = AnthropicProvider(
                            self.config.llm_config.get('api_key', ''),
                            self.config.llm_config.get('model', 'claude-3-haiku-20240307')
                        )
                    elif provider_type == 'ollama':
                        from .query_processor import OllamaProvider
                        llm_provider = OllamaProvider(
                            self.config.llm_config.get('model', 'llama2'),
                            self.config.llm_config.get('base_url', 'http://localhost:11434')
                        )
    ...
    def handle_ambiguous_query(self, context: QueryContext) -> List[str]:
        ...
        if any(word in query_lower for word in ['last', 'this', 'current', 'recent']):
            if 'month' not in query_lower and 'year' not in query_lower and 'week' not in query_lower:
                suggestions.append("Specify a time period (e.g., 'last month', 'this year', 'last week')")
        ...
        if 'cost' in query_lower and not any(word in query_lower for word in ['total', 'breakdown', 'by service']):
            suggestions.extend([
    ...
    def validate_query_complexity(self, query_params: QueryParameters) -> Dict[str, Any]:
        """
        ...
        if self.aws_client:
            ...
            return {"complexity_level": "unknown", "warnings": ["AWS client not initialized"]}
